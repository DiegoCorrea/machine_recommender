...
class PreProcessing:
    ...
    def link_user_and_songs(self):
        users_preferences_df = pd.read_csv(
            os.getcwd() + '/data/original_set/train_triplets.txt',
    ...
    def save(self):
        self.__song_df.to_csv(os.getcwd() + self.__clean_data_path + 'songs.csv', index=False,
                              columns=['id', 'title', 'artist', 'album', 'gender'])
        self.__users_preferences_df.to_csv(os.getcwd() + self.__clean_data_path + 'play_count.csv', index=False,
